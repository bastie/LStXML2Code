...
class PAPTreeEncoder : Encoder {
  /// - Parameters
  /// - Parameter value CommonAST
  public func encode(_ value: Encodable) throws -> AbstractTree {
  ...
  let encoding : PAPTreeEncoding
  public init(encoding worker : PAPTreeEncoding){
    self.encoding = worker
  }

  var data: AbstractTree {
    return container?.storage ?? AbstractTree()
  }
  ...
  class SingleValueContainer: SingleValueEncodingContainer {
    ...
    func encode<T>(_ value: T) throws where T : Encodable {
      ...
      if value is PAPTree {
        self.storage = try self.encoding.encode(value as! PAPTree)
      }
    }
    ...
    var encoding : PAPTreeEncoding
    init(codingPath: [CodingKey],
         userInfo: [CodingUserInfoKey : Any],
         encoding : PAPTreeEncoding)
    {
